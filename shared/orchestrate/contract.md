...
**発動条件は機能OR**（オーナー裁定 2026-07-18・旧3条件ANDを撤廃）: ①計画に中断が組み込まれている②受入が多段に連鎖する③複数repoの書込みを調整する④裁定の検証可能な証跡が必要、のいずれかが**着手時の作業構造として確定**している戦役（campaign）だけで使う。重装備はwriter委譲・受入裁定・Phase gate・H操作の4関節だけに課す（比例原則）。技法（fan-out・反証・Critic）の参照は全レーン可。対象projectの`docs/`にある生きた計画/TODOが正本であることを最初に確認する。
