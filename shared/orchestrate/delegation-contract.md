...
- 同種ホストへの委譲（Claude→Claude subagent、Codex→入れ子Codex）は通常の委譲先として使える。委譲呼び出しはmodel・effortを明示したものだけを許す——親と同値の指定は可、省略による無自覚な継承は不可。委譲先とmodel・effortの選定は、その作業に必要な能力とコストを見積もった上での判断だけを許し、委譲するのはその利益が準備・受入コストを上回る時だけ（オーナー裁定 2026-07-18）。
