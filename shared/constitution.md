...
- **統括レーンの発動は機能条件のOR**（オーナー裁定 2026-07-18。旧「複数repo・複数Executor・複数Phaseの3条件AND」を撤廃）: 次のいずれか1つでも**着手時点の作業構造として確定している**なら統括レーンで着手する——①計画に中断が組み込まれている（承認待ち・外部完了待ち・複数波の波間停止）②受入が多段に連鎖する③複数repoの書込みを調整する④裁定の検証可能な証跡が必要。頭数・repo数・Phase数・「長くなりそう」という予測は発動条件にしない。予定外にセッションが途切れた通常レーン作業は、遡って統括化せずhandoff（現在地をplanへ1行＋バトン）で閉じる。
- **統括レーン内の比例原則**: 重装備文書（Packet/Report・Control記録）が必須なのは4つの関節——戦役Taskのwriter委譲・受入裁定・Phase gate・H操作——だけ。統括自身の直接処理、read-only補助呼び出し（反証・調査・レビュー）、queueで束ねた小粒消化には課さない（証跡はgate evidenceとdocsで足りる）。
- **技法と儀式の分離**: orchestrateの技法（並列fan-out・重複統合・反証・網羅性Critic）は通常レーンでも自由に使ってよい。統括レーン専用なのはControl儀式（Elastic Control lifecycle、Packet/Report、受入・回収契約）の方であり、`orchestrate`正典はその時だけ適用する。H操作の承認と高リスク操作の説明義務は、レーンに関係なく「ツールと権限」のとおり適用する。
