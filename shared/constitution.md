...
8. **工場CIは4環境で同じfullを並列実行する**: dotagents工場管理製品の最終CIは、self-hostedのmacOS native・Linux native・Windows native・WSL2で同時に開始し、4環境すべてで同じ製品full testを実行する。OSごとの役割分散はしない。runnerの依存ツールは各OSの標準システム環境へ導入し、個人用PATHや特殊な端末状態をCIの前提にしない。
