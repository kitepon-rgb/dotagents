...
test('未知・不足引数は台帳を作らず拒否し、Windows ACL契約を実装に固定する', async (t) => {
  ...
  assert.match(source, /DirectorySecurity/); assert.match(source, /FileSecurity/); assert.match(source, /SetAccessRuleProtection\(\$true,\$false\)/); assert.match(source, /ownerOnlyAcl\(path\)/);
});
