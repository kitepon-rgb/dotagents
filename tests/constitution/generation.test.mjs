...
test("実repoの共通契約とhost固有契約を交差させず保持する", async () => {
  ...
  assert.ok(common.split("\n").includes("本書の「<name> runbook」は `~/.claude/runbooks/<name>.md`（Codexは `~/.codex/runbooks/<name>.md`・実体はdotagents `shared/runbooks/`）を指す。"));
  assert.ok(common.split("\n").includes("- **調査と出力を還流させる**: 調べた外部仕様・文献は`rag/`へ、価値ある出力（回答・監査ダイジェスト・図解）は内容に応じて`rag/`または`docs/`へ還流して複利で育てる。保存手順（MarkItDown化・raw/コンパイル分離・出典/取得日/確度・INDEX追記）と月次衛生は knowledge-return runbook に従う。"));
