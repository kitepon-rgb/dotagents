...
mkdir -p "$WINDOWS_HOME/.ssh" "$WINDOWS_HOME/.codex"
...
windows_hooks_before="$(cksum "$WINDOWS_HOME/.codex/hooks.json")"
...
[ "$(cksum "$WINDOWS_HOME/.codex/hooks.json")" = "$windows_hooks_before" ] || fail 'Windows Codex hooksを変更した'
...
[ "$(cksum "$WINDOWS_HOME/.codex/hooks.json")" = "$windows_hooks_before" ] || fail '再実行でWindows Codex hooksを変更した'
[ "$(grep -Fxc "$public_key" "$WSL_HOME/.ssh/authorized_keys")" -eq 1 ] || fail 'Windows公開鍵を重複登録した'

run_configure --check
[ "$(cksum "$WINDOWS_HOME/.codex/hooks.json")" = "$windows_hooks_before" ] || fail 'checkでWindows Codex hooksを変更した'
