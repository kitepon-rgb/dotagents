...
- **3系統に拡張（オーナー裁定 2026-07-12）**: 当初2系統（配置 C1／TODO C2-C3）に、着手案内 C4 を追加。現行 C4 はセッション初回と compact 後だけの INFO で、詳細な義務はグローバル正典を参照させる。
...
2. **INFO に限定**: additionalContext は観測事実とグローバル正典への参照だけにする。Hook から permissionDecision、ask、deny、block は返さない。
...
5. **エスケープ**: `DOTAGENTS_PLACEMENT_GATE=off`／`DOTAGENTS_TODO_GATE=off`／`DOTAGENTS_ONSET_GATE=off` で各INFO経路を無効化する。
...
### 旧裁定メモ（初版の経緯・Phase 6 が supersede）
...
- **xhigh/max/ultra の Hook 判定**: 初版では強制候補だったが、現行はすべてグローバル正典へ委ね、Hook では検査しない。
...
- **スロットル**: セッション1回。compact 後は状態をクリアして1回だけ再武装する。
...
- **出力**: 棚卸しの観測事実と、グローバル `CLAUDE.md` / `AGENTS.md`「計画文書の作法」への参照だけを INFO で返す。現在の依頼範囲は変更しないと明記する。

#### C3: `bin/todo-gate-hook.sh stop`（プラン正本の更新忘れ）
...
- **出力**: Stop では context 注入も block も行わない。観測事実と正典参照を pending に保存し、次の自然な UserPromptSubmit で1回だけ配送する。`DOTAGENTS_TODO_GATE=block` に昇格動作はない。

#### C4: `bin/onset-gate-hook.sh`（着手案内 INFO）

- **背景**: C1（配置ゲート）は委譲ツールが呼ばれた時しか発火せず、「委譲すべきなのに Edit/Write で抱え込む」失敗（2026-07-05／2026-07-11 実被弾）を捕まえられない＝反証 #2 で v2 送りにした穴。Throughline の Claude 要望＋オーナー 2026-07-11 裁定で本体へ格上げ。着手・委譲の一次定義はグローバル正典「作業レーンと統制」（2026-07-16裁定: 委譲は統括昇格ではなく、既定委譲は廃止＝利益がある時だけ）。
- **目的（2層）**: ①**抱え込み防止**＝物量・隔離・quota残・準備コストの観点で委譲を検討させる。②**偽準拠防止**＝配置宣言を 02_models.md を開いて file:line 引用で写す（暗記の例文丸写しによる候補漏れの再発を潰す）。
- **イベント×matcher**: `UserPromptSubmit`（matcher なし）。
- **強度**: INFO のみ。詳細な義務はグローバル正典へ委ね、この Hook 自体が作業・文書・委譲・依頼範囲を増やさないと明記する。
- **頻度**: セッション初回だけ。compact 後に1回だけ再武装する。
- **共存**: 既存 caveat/throughline の UserPromptSubmit と hooks はマージ実行（P2/P5 実証）。
- **文言**: グローバル `CLAUDE.md` / `AGENTS.md` の「計画文書の作法」「モデルとエフォート」と orchestrate skill への短い参照。
- **無効化**: `DOTAGENTS_ONSET_GATE=off`。
- **Codex ミラー**: X5 として同じ頻度・強度で実装する。
...
- **X2 `pre-tool-use`**（update_plan / spawn_agent のみ拾う）:
  - **update_plan 初回かつ step 数 ≥4** → 永続プランの正典参照を INFO で返す（セッション1回）。
  - **update_plan 全 step completed** → 進捗反映と完了文書の扱いを正典へ案内する INFO（セッション1回）。
  - **spawn_agent** → 初回だけ配置・routing の正典参照を INFO で返す。引数検査・deny は行わない。
- **X3 `user-prompt-submit`**: X4 pending の1回配送＋X5 セッション初回案内。compact 後に1回再武装。
- **X4 `stop`**: C3 ミラー。rolling baseline で検出し、Stop では出力せず pending 保存する。
...
| INFO（additionalContext） | C1/X2 初回委譲、C2/X1 棚卸し、plan gate/X2 update_plan、C4/X5 セッション初回、C3/X4 pending 配送 |
| ask / deny / block | 使用しない |
...
| ~~統括の抱え込み検出（Stop 時に直接編集量を報告）~~ | C4 の正典参照 INFO として採用。ただし反復注入は暴走誘発リスクがあるため、現行はセッション初回＋compact後1回に限定 |
...
- [x] `bin/onset-gate-hook.sh`（C4・セッション初回＋compact後のINFO）＋空打ちテスト
...
- [x] 旧実火観測: 初版の deny / ask / block 到達を確認（現行 Phase 6 ではすべて廃止）
...
- **空打ち**: 個別パラメータに関係なく初回だけINFO、2回目はゼロバイト、Stopはpending保存のみ、次のUserPromptSubmitで1回配送、compact後に再武装、各`off`で沈黙を assert。
- **実火**: 通常セッションで固定文が反復注入されないこと。回帰: plan-gate・caveat・throughline の既存 hook が従来どおり発火。
- **常設**: verify-install の配線検証（手挿し忘れの穴を既存 plan-gate ごと塞ぐ）。

## リスクと開いた問い

- P3 の additionalContext 単独注入は実測済み。現行 C1 はこの INFO 経路だけを使う。
- PreToolUse はセッション初回 INFO だけなので反復注入しない。compact 後の再武装が意図せず同一セッションで多発しないことを実火で確認する。
- C3 の誤検知（雑務 dirty）は停止を妨げず pending を次の自然な UserPromptSubmit へ1回だけ配送する。不要なら `DOTAGENTS_TODO_GATE=off` で無効化できる。
...
## 旧オーナー裁定（2026-07-12・Phase 6 が supersede）
...
## Phase 6 — INFO化と反復注入の解消（2026-07-12・旧裁定を supersede）
...
- [x] C3/X4: Stop からの context 注入と block を廃止し、pending に保存して次の自然な UserPromptSubmit で1回だけ INFO 配送する
- [x] Claude / Codex smoke を新契約へ更新し、旧 deny / ask / Stop block が消えたことを回帰検証する
- [x] `docs/03_settings-fragments.md` / `docs/05_codex-fragments.md` / `rag/hooks/callout-hooks-firing-behavior.md` を現行仕様へ更新する
- [x] `make lint` / `./install.sh` / `./bin/verify-install.sh` を green にし、配布先 symlink の実行で初回 INFO・同セッション2回目ゼロバイトを確認する
