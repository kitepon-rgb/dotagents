# 02_models —  役割→現行最強モデルの対応表（唯一の参照点）
...
**品質エスカレーションは統括の裁量（安さは既定であって強制ではない）**: 委譲物を検証して品質に納得しない時、統括（ベル）の判断で**上位モデル（Sonnet 5 → Opus → Fable 自身）へ引き上げて再実行してよい**。コスト最適化が品質を人質にしてはならない——「安く済ませる」より「正しく仕上げる」が上位。エスカレーションした事実と理由は残す（次回の配置判断のため）。下向き最適化（上記の優先順位）と、この上向きエスカレーションは、統括が「委譲物は必ず自分で検証」した結果として両立する。
