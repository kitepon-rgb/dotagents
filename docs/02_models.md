...
- **aiterm `codex_agent` は端末 config を丸ごと継承・model 引数なし**＝`reasoning_effort` 引数必須・モデル既定は親と同じ（aiterm 改修依頼中: docs/plan_gpt56-rewiring.md）。
- **aiterm の grok/composer は隔離設計（OAuth のみ共有）**＝ピン継承問題なし。ただし `grok_agent` のハードコード `--model grok-build` は stale（現行カタログに不在。改修依頼中）。**grok の `--effort` は headless（`grok -p`）専用で対話 TUI では無視される**。
