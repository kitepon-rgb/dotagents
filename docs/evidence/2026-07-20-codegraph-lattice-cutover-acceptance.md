...
- Lattice release: `@quolu/lattice@0.7.3`
- Lattice commit: `a10ad4ac62e92e7f1d29c2e9040cff25d862e0ab`
...
|---|---|---|---|---|---|
| Mac | 0.7.3 | `lattice`登録、実Claude sessionでtool実行 | 撤去、0件 | 旧73 recordをowner-only rollback backupへ移動、現0件 | `64f9fe7b`、success |
| main-server | 0.7.3 | `lattice`登録・接続・JSON-RPC smoke | 撤去、0件 | 旧recordなし | `b368e834`、success |
| FOX WSL2 | 0.7.3 | `lattice`登録・接続・JSON-RPC smoke | 撤去、0件 | 旧1 recordをrollback backupへ移動、現0件 | `347f1ed2`、success |
| FOX Windows | 0.7.3 | `lattice`登録・接続、sensor init/sync | 撤去、0件 | 旧recordなし | `da5f9acc`、success |
...
```json
{"provider":"lattice","sensor_owner":"lattice","version":"0.7.3-lattice.1","mode":"daemon"}
```
...
BugHub matrixでは4 active hostすべてCodegraph=`not_applicable`、Lattice=`installed@0.7.3`。`q22-canary`はhost自体が
`retired`なので、旧Codegraph=`installed`を移行履歴として保持しcurrent運用集合へ含めない。

## 0.7.3配布修理

0.7.0 tarballに残った旧`codegraph.js`実行物を除去する過程で、0.7.1はprivate runtime欠落、0.7.2は
helper欠落を検出した。両版を成功扱いせず、`lattice-sensor.js`と必要helperだけを配布し
`codegraph.*`実行物を除外した0.7.3を公開した。registry tarball、4hostの`init`/`sync`、実fixtureの
8互換面（search/callers/callees/impact/node/explore/status/files）を再検証し、全hostを0.7.3へ更新した。
