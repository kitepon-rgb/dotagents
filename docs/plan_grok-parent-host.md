...
Wave 5 FOX WSL2 H適用（2026-08-16。repo=6a7b8c9）。backup=`~/Archives/dotagents-pre-wave5h-fox-20260816-141919.tar.gz`。`install.sh --profile official` で `~/.grok` の rules/runbooks/skills/agents/hooks を本リポ向きsymlink。`apply-grok-config --apply` は compat.agents/hooks=false と工場MCP 6を追加（`[models]` は grok-4.5 / high のまま）。inspect: 有効global user ruleは`~/.grok/rules/AGENTS.md`だけ。工場skill 4列挙、bundled残る。工場hook enabled 9件。Claude settings由来25件はdisabled。vercel plugin hook 1件はenabledのまま（settings.json由来ではない）。工場MCP 6はconfigToml。doctor: caveat/lattice/aiterm/gpt_connector/codex-sidecar は handshake OK。aishell は `command not found`（typed。WSLはmatrixどおりunsupported）。FOX新規Grok session受入は未実施。Windows nativeは未実施。Wave 6は別H。
