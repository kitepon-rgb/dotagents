...
Mac側のWave 5 Hは閉じた（2026-08-16 session `01a0091e`）。FOX WSL2の新規Grok session受入も閉じた（2026-08-16 session `01a00964`）。FOX Windows nativeの適用後新規session受入も閉じた（2026-08-16 session `01a00999`）。工場の4席（Mac / Windows native / WSL2 / Linux）は全部本線。Windows nativeを対象外にした旧裁定8は2026-08-16に破棄。Windows nativeのGrok親配線とlogin済みapplyは着地（2026-08-16）。製品未対応面は`unsupported`のまま残してよい。残HはLinux / main-serverでGrokを親にするときの人の目。Wave 6は別H。
