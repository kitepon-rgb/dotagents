...
Wave 1〜5がdotagentsで閉じる範囲。MacとFOX WSL2のsession受入は着地済み。Windows nativeのGrok親配線とlogin済みapplyはこの席で着地。この窓（session `01a0097e`）は適用前に起動したので、起動時handshakeを適用後のsession受入に数えない。残HはFOX Windows nativeの適用後新規sessionと、Linux / main-serverでGrokを親にするときの人の目。Wave 6は製品repoであり、本計画の完了条件に含めず導線だけ置く。
...
**H:** MacとFOX WSL2の新規Grok session受入は着地済み。FOX Windows nativeの配線とlogin済みapplyは着地。残HはFOX Windows nativeの適用後新規session。Linux / main-serverでGrokを親にするときの人の目も残る。
...
Mac側のWave 5 Hは閉じた（2026-08-16 session `01a0091e`）。FOX WSL2の新規Grok session受入も閉じた（2026-08-16 session `01a00964`）。工場の4席（Mac / Windows native / WSL2 / Linux）は全部本線。Windows nativeを対象外にした旧裁定8は2026-08-16に破棄。Windows nativeのGrok親配線とlogin済みapplyは着地（2026-08-16）。この窓の起動時handshakeは適用前なのでsession受入に数えない。残HはFOX Windows nativeの適用後新規session。製品未対応面は`unsupported`のまま残してよい。Wave 6は別H。
