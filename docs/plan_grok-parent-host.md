...
| 製品 | 内容 | 今の状態 |
|---|---|---|
| Throughline | Grok turnのcapture / restore / handoff | `01a00b38`はsession_start / user_prompt_submitがsuccessで`grok:`行は実在。L2とprocess-turnは当turn未完了で未観測。成功条件未達。restore/handoff実機と他席は未了 |
...
Wave 6 Throughline Grok hook capture 人の目（2026-08-17 session `01a00b38-87ea-7670-8f7d-a9fe937263c5`、このDesktop窓。前session `01a00b2f` の見た目は数えない）。Throughline origin/mainは`4b6bf72`、dotagents origin/mainは`f8889fd`。`~/.grok/hooks/throughline.json`のcommandは`/opt/homebrew/bin/node` + Throughlineの`bin/throughline.mjs` + subcommand。factory.jsonは未改変。このsessionの`updates.jsonl`で`global/throughline:session_start`はsuccess 37ms、`user_prompt_submit`はsuccess 34ms。127 / command not found は無い。`events.jsonl`にthroughline痕跡は無い。`throughline status`と`~/.throughline/throughline.db`に`grok:01a00b38-87ea-7670-8f7d-a9fe937263c5`行はある。bodiesのL2 user/assistantは0件。当turn未完了のため`process-turn`（Stop）は未観測。Claude/Codexの既存sessionとsettingsは触っていない。npm未公開・restore/handoff実機・Spotter着手はしていない。成功条件（この新規sessionの`grok:`行とL2）はL2未達。次はこのsessionの次turnでStop / L2を実測する。
