...
- 初回導入と再適用はMacの`setup-macos-factory.sh`、WSL2の`setup-wsl-factory.sh`、Windows nativeの`setup-windows-native-factory.ps1`が所有する。3入口はdeployment contractの製品集合だけを共有し、OS固有のLaunchAgent／cron／Task Scheduler、config、hook、credential、delivery receiptを共有fileへ集約しない。各入口は定期更新を冪等登録し、`verify-install`、固定15製品のfresh wire v7 report、BugHub delivery receiptまでを受け入れる。
