...
- wire: v6固定14製品へpeertableを加えた`V7_PRODUCT_IDS`固定15製品（`lib/factory/v7.mjs`・[wire v7設計](wire-v7-design.md)・[ADR 0127](adr/0127-wire-v7-peertable-enrollment.md)）。ServerManager/BugHubのv7 ingestはserver-firstでdeploy・`FACTORY_V7_INGEST_ENABLED=true`済み、cutoverはmac-kiteだけ完了。main-server自身のhost分とFOX 2host分は残作業で、その間はhostごとにv6とv7が並存する。
