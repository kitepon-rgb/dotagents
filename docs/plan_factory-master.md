...
| 24 | `DONE` | AIShellを工場コア製品へ編入した。wire v5で固定13製品へenrollし、4 host（mac / server / wsl / windows-native）をcutover。AIShellはmac=required、他=unsupported。v4 endpointはretention期間のため据置 | AIShell → dotagents → ServerManager / A0〜A6完了 |
