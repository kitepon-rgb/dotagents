...
2. **GitHub が真実の源**: プロジェクトは端末を跨いで存在する。**全てのプロジェクト作業は fetch→照合→整合させてから**。作業後はpushで真実を返す（本原則は、dotagentsと自作コア11製品の正規repoに対する恒久push裁定である。第三者製品・基盤toolchainには適用しない。認定手順は憲法git鉄則に従う）。
