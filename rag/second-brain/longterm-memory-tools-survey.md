...
2. うちには既に**5層の長期記憶**があり、全部 git 同期の素の md（罠=caveat〔リポ化済み〕・研究=rag/・決定=docs/・作法=CLAUDE.md 正本・進捗=docs/TODO.md）＋端末メモリ（Claude Code ネイティブのファイルベース記憶）＋Throughline（セッションのバトン）。**保存層の欠落は無い**。
