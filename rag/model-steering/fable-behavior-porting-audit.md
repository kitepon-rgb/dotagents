...
2. **次の宿題＝憲法の選択的スリム化**: L174 を踏まえ、「挙動を1つずつ名指しする steering 列挙」は Fable の指示追従力（L53）を信じて圧縮できる余地。ただし **git/rsync/stash 系の実被弾ハザード記憶（CLAUDE.md L84-96）は対象外**（Fable の指示追従と無関係＝残す）。実施は憲法改訂＝契約クリティカルにつき別途オーナー裁定。
