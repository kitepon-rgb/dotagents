...
- [hooks/callout-hooks-firing-behavior.md](hooks/callout-hooks-firing-behavior.md) — 呼びかけ hook 群の発火挙動実測（Claude C1-C4／Codex X1-X5）: PreToolUse additionalContext 到達・子ツールも親発火・settings.json hot-reload・Codex async 非対応/trust/Stop block 反映・状態ファイル形式。将来 hook 改修の一次参照（2026-07-12・確度 reproduced・実火観測）
...
- [codex/codex-full-support-foundations.md](codex/codex-full-support-foundations.md) — dotagents Codex 全対応の公式仕様基盤: 9監査面、公式 skill 面 `$HOME/.agents/skills`、legacy `~/.codex/skills` 実測、import は同期でなく検出器、plugin は二重管理防止を実証後に裁定（2026-07-12・確度高）
