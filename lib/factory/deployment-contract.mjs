...
export const CURRENT_WIRE_PRODUCT_IDS = Object.freeze([
  ...MANAGED_PRODUCT_IDS.slice(0, 8), 'servermanager', 'claude-code', 'codex-cli', 'grok-build', 'aishell', 'observer', 'peertable',
]);
...
const BASE_REQUIRED = Object.freeze(['caveat', 'throughline', 'spotter', 'lattice', 'markitdown', 'gpt-connector', 'aiterm-mcp', 'codex-sidecar', 'peertable']);
...
export function hostProjection({ profile, os, arch, macosMajor = null }) {
  ...
  expected.observer = profile === 'mac' ? 'required' : 'unsupported';
...
export function npmPackagesForHost({ os, arch }) {
  ...
  // aiterm-mcp は Windows native だけ一時除外（2026-08-16 オーナー裁定）。
  // Windows 基盤を WSL 橋から native psmux へ移行中で、端末には aiterm-winfix
  // feat/windows-native-psmux のローカルビルドを `npm install -g .` で先行適用している。
  // registry 版 (WSL 橋) で上書きさせない。psmux 上流 PR
  // https://github.com/psmux/psmux/pull/577 の merge → aiterm の native 版 release 後に
  // この除外を戻す（戻し忘れ防止は watch-psmux-pr-577 監視タスクが追跡）。
  const packages = BASE_NPM_PACKAGES.filter(
    (name) => !(canonicalOs === 'win32' && name === 'aiterm-mcp'),
  );
  if (canonicalOs === 'darwin') packages.push('@quolu/observer');
