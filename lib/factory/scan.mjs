...
async function thirdParty(productId, options) {
  ...
  if (productId === 'oracle') {
    const result = await run('oracle', ['doctor', '--json'], options);
    let doctor;
    try { doctor = result.ok ? JSON.parse(result.stdout) : null; } catch { doctor = null; }
    const machineReadable = doctor && typeof doctor === 'object' && !Array.isArray(doctor);
    product.checks.push({ check_id: 'doctor', status: machineReadable ? 'pass' : 'unverified' });
  }
