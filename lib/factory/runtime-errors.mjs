...
export function validateAcknowledgementBundleV4(value, expectedReportId) {
  exact(value, ['schema_version', 'report_id', 'acknowledgements'], '$');
  if (value.schema_version !== '4.0' || typeof value.report_id !== 'string' || value.report_id !== expectedReportId || !Array.isArray(value.acknowledgements) || value.acknowledgements.length > 3) fail('ack_bundle_v4');
...
export async function acknowledgeRuntimeErrorsV4(bundle, { runner = run, cwd, env } = {}) {
