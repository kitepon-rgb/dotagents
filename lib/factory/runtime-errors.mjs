...
function parseThroughline(value) {
  ...
  if (value.schema !== 'throughline.runtime_errors.v1' || value.product !== 'throughline') fail('throughline_schema');
  ...
  if (acknowledged > high || next > high) fail('throughline_cursor');
  ...
  if (!['enabled', 'disabled'].includes(value.diagnostics.collection)
    ...
    || typeof value.diagnostics.truncated !== 'boolean') fail('throughline_diagnostics');
  ...
  if (value.diagnostics.collection === 'disabled') {
    if (value.diagnostics.status !== 'not_applicable' || high !== 0 || acknowledged !== 0 || next !== 0
      || value.runtime_errors.length !== 0 || value.resolutions.length !== 0) fail('throughline_disabled');
    return emptyProjection('throughline', 'collection_disabled');
  }
  if (value.diagnostics.status !== 'ready') fail('throughline_status');
  const runtimeErrors = value.runtime_errors.map((record, index) => projectThroughlineRecord(record, index));
  ...
  if (value.diagnostics.pending_count > value.diagnostics.total_count
    ...
    || (selectedCount > 0 && next === 0)) fail('throughline_cursor');
  assertUniqueAndDisjoint(runtimeErrors, resolutions);
  return readyProjection('throughline', next, runtimeErrors, resolutions);
}
...
function projectThroughlineRecord(record, index) {
  ...
  if (record.fingerprint !== nulFingerprint('throughline', record.component, record.error_code, record.message_template)) {
    fail(`${path}.fingerprint`);
  }
  return common;
}
