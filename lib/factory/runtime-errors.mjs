...
export async function acknowledgeRuntimeErrors(bundle, { runner = run, cwd, env } = {}) {
  ...
  for (const acknowledgement of bundle.acknowledgements) {
    ...
    if (!result?.ok) throw contractError('ack_failed');
    validateAcknowledgementResponse(acknowledgement.product, acknowledgement.cursor, result.stdout);
  }
}

// v2 wire is deliberately separate from v1: its only runtime producer is gpt-connector.
export function validateAcknowledgementBundleV2(value, expectedReportId) {
  exact(value, ['schema_version', 'report_id', 'acknowledgements'], '$');
  if (value.schema_version !== '2.0' || typeof value.report_id !== 'string' || value.report_id !== expectedReportId || !Array.isArray(value.acknowledgements) || value.acknowledgements.length > 1) fail('ack_bundle_v2');
  for (const acknowledgement of value.acknowledgements) {
    ...
    if (acknowledgement.product !== 'gpt-connector' || acknowledgement.command !== 'gpt-connector' || !Array.isArray(acknowledgement.args) || acknowledgement.args.length !== 4 || acknowledgement.args[0] !== 'runtime-errors' || acknowledgement.args[1] !== 'ack' || acknowledgement.args[2] !== String(expected) || acknowledgement.args[3] !== '--json') fail('ack_command_v2');
  }
  return value;
}
...
export async function acknowledgeRuntimeErrorsV2(bundle, { runner = run, cwd, env } = {}) {
  ...
  for (const acknowledgement of bundle.acknowledgements) {
    const result = await runner('gpt-connector', acknowledgement.args, { cwd, env, timeoutMs: COMMAND_TIMEOUT_MS, maxOutputBytes: MAX_OUTPUT_BYTES });
...
function validateAcknowledgementResponse(product, expectedCursor, stdout) {
  ...
  if (product === 'caveat' || product === 'throughline') {
    ...
    parseSpotter(value); acknowledged = cursor(value.acknowledged_through, '$.acknowledged_through');
    ...
    parseCodexSidecar(value); acknowledged = cursor(value.factoryRuntimeErrors.acknowledged_through, '$.factoryRuntimeErrors.acknowledged_through');
...
export async function collectRuntimeErrors(product, {
  ...
  if (!result?.ok) {
    ...
    throw contractError('command_failed');
  }
  const snapshot = parseJsonObject(result.stdout);
  const projection = adapter.parse(snapshot);
...
function contractError(code) {
  return Object.assign(new Error(`runtime error adapter contract failed: ${code}`), { code: 'E_FACTORY_RUNTIME_ERRORS', reason_code: code });
}
