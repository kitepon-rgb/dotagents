...
const SAFE_CONTEXT_ALLOWLIST = Object.freeze(
  Object.fromEntries(PRODUCT_IDS.map((id) => [id, new Set()])),
);
...
export function validateConfig(config) {
  ...
  if (!['server', 'mac', 'wsl', 'windows-native'].includes(config.host.profile)) {
...
export function validateReport(report) {
  ...
  if (report.schema_version !== '1.0' || report.report_mode !== 'full') {
  ...
  if (!['server', 'mac', 'wsl', 'windows-native'].includes(report.host_profile)) {
  ...
  if ({ mac: 'darwin', server: 'linux', wsl: 'linux', 'windows-native': 'windows' }[report.host_profile]
    !== report.platform.os) {
  ...
  exactKeys(report.products, PRODUCT_IDS, 'products');
  if (!PRODUCT_IDS.every((id) => isObject(report.products[id]))) {
    throw new Error('productsは固定9製品をすべて含む必要があります');
  }
