...
function validateTask(value, stored = true) {
  ...
  exact(value.context_policy, ["share_objective", "share_current_candidate", "share_existing_findings", "share_failed_approaches", "share_test_results"], "task.context_policy");
  for (const [key, current] of Object.entries(value.context_policy)) if (typeof current !== "boolean") fail("INVALID_SCHEMA", `task.context_policy.${key} must be boolean`);
