...
const MANIFEST_SCHEMA = "dotagents.orchestration-control.v15";
...
const TRANSITION_OPERATIONS = [
  ...
  "worker-observe", "worker-report-import", "consultation-observe", "worker-accept", "worker-reject", "task-finalize", "control-finalize", "control-archive",
];
...
export function validateManifest(value) {
  exact(value, ["schema_version", "record_revision", "control_id", "status", "declaration", "continuation", "durability", "budget", "role_effect_policy", "document_refs", "registry_observations", "tasks", "task_cancellations", "worker_runs", "consultations", "task_finalizations", "control_finalization", "transition_receipts", "last_update"], "manifest");
  ...
  for (let index = 0; index < value.transition_receipts.length; index++) {
    ...
    if ((receipt.operation === "placement-reserve") !== (receipt.subject_digest !== null)) fail("INVALID_SCHEMA", "transition subject digest is invalid for operation");
  }
  ...
  unique(value.registry_observations, "registry_observation_id"); unique(value.tasks, "task_id"); unique(value.task_cancellations, "task_id"); unique(value.worker_runs, "worker_run_id"); unique(value.consultations, "consultation_id"); unique(value.task_finalizations, "task_id");
...
export async function init(input) {
  ...
  return withLock(identity, async (paths) => {
    ...
    const manifest = {
      ...
      budget: structuredClone(input.budget), role_effect_policy: structuredClone(ROLE_EFFECT_POLICY), document_refs: [...input.document_refs], registry_observations: [], tasks: [], task_cancellations: [], worker_runs: [], consultations: [], task_finalizations: [], control_finalization: null,
...
function briefManifest(manifest) {
  ...
  return {
    schema_version: "dotagents.orchestration-status-brief.v2",
    ...
    counts: {
      ...
      worker_runs: manifest.worker_runs.length, consultations: manifest.consultations.length,
    },
    active: { worker_runs: activeWorkers, consultations: activeConsultations },
    cancellations: {
...
export async function resumeCheck(input) {
  ...
  return withLock(identity, async (paths) => {
    ...
    for (const run of manifest.worker_runs.filter((entry) => WORKER_NONTERMINAL.has(entry.state))) {
      let workspace;
      try { workspace = await gitIdentity(run.workspace.worktree_root_realpath ?? run.workspace.git_dir_realpath); }
      catch (error) {
        if (!(error instanceof ControlRecordError)) throw error;
        blocking.push(resumeIssue("worker-workspace-unavailable", "worker-run", run.worker_run_id)); continue;
      }
      if (!sameWorkspaceIdentity(run.workspace, workspace)) blocking.push(resumeIssue("worker-worktree-generation-changed", "worker-run", run.worker_run_id));
      const expectedHead = run.workspace.head_at_reservation ?? run.workspace.head_at_record;
...
async function evaluatePlacementCandidate({ manifest, manifests, task, candidate, evaluatedAt }) {
  ...
  try {
    ...
    stored = structuredClone(synthetic); delete stored.workspace_cwd; stored.workspace = workspaceObject(workspace); stored.recorded_workspace_fingerprint = null; stored.baseline_workspace_fingerprint = null;
...
export async function workerRunRecord(input) {
  ...
  return mutation(input, { operation: "worker-run-record", subjectKind: "worker-run", subjectId: input.worker_run.worker_run_id, previousState: null, nextState: "planned", evidence: [input.worker_run.execution_verification.evidence] }, async (manifest, manifests) => {
    ...
    const stored = structuredClone(input.worker_run); delete stored.workspace_cwd; stored.workspace = workspaceObject(workspaceIdentity);
...
export function workerReportTemplateForPacket(packet) {
  exact(packet, ["schema_version", "packet_digest", "control_id", "record_revision", "task", "worker", "executor", "workflow_id", "workspace", "scope", "classification", "effect", "capabilities", "budget", "lineage", "validation", "non_goals", "known_traps", "report_schema_id", "report_template"], "delegation_packet");
...
export async function importWorkerReport(input) {
  ...
  return mutation(input, (before) => {
    ...
    if (run.write_mode !== "none") {
      ...
      const actualChanged = changedPaths(run.baseline_workspace_fingerprint, fingerprint);
