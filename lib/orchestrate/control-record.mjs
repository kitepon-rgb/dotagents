...
const MANIFEST_SCHEMA = "dotagents.orchestration-control.v25";
...
const claudeHandle = (value) => { exact(value, ["session_id"], "executor_handle"); identifier(value.session_id, "executor_handle.session_id"); };
...
const executorContract = (adapter_id, workflow_id, handle_schema_id, external, nullable_handle, validate_handle, validate_capabilities = noCapabilityConstraint) => ({ adapter_id, contract_version: "v1", workflow_id, handle_schema_id, external, nullable_handle, validate_handle, validate_capabilities });
export const EXECUTOR_CONTRACT_REGISTRY = createExecutorContractRegistry([
  executorContract("parent", "direct", "parent.correlation.v1", false, false, parentHandle),
  executorContract("codex-native", "native-subagent", "codex-native.agent-path.v1", false, true, nativeHandle),
  ...["auditor", "explore", "generate", "opinion", "review", "risk-check"].map((workflow) => executorContract("codex-sidecar", workflow, "codex-sidecar.synchronous.v1", true, true, rejectSynchronousHandle, sidecarReadonlyCapabilities)),
  executorContract("codex-sidecar", "work", "codex-sidecar.idempotency-key.v1", true, false, sidecarHandle, sidecarWorkCapabilities),
  executorContract("aiterm", "interactive-session", "aiterm.session.v1", true, true, aitermHandle),
  executorContract("claude-native", "native-subagent", "claude-native.session.v1", true, true, claudeHandle),
]);
...
export function validateManifest(value) {
  ...
  for (let index = 0; index < value.transition_receipts.length; index++) {
    ...
    if (["placement-reserve", "worker-run-record", "worker-workspace-bind", "campaign-record", "phase-gate-record", "artifact-record", "artifact-status-record", "approach-family-record", "approach-family-block", "approach-family-reopen"].includes(receipt.operation) !== (receipt.subject_digest !== null)) fail("INVALID_SCHEMA", "transition subject digest is invalid for operation");
  }
...
function briefManifest(manifest) {
  ...
  return {
    schema_version: "dotagents.orchestration-status-brief.v6",
...
export async function resumeCheck(input) {
  ...
  return withLock(identity, async (paths) => {
    ...
    return {
      schema_version: "dotagents.orchestration-resume-check.v5", checked_at: new Date().toISOString(),
...
function assertControlReadyForFinalization(manifest, code) {
  ...
  if (!workerReady || !consultationReady || !tasksFinalized || !campaignsReleased || !phaseGateComplete(manifest)) fail(code, "control is not ready for finalization");
}
