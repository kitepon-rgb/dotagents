...
const MANIFEST_SCHEMA = "dotagents.orchestration-control.v22";
...
const TRANSITION_OPERATIONS = [
  ...
  "worker-workspace-bind", "worker-observe", "worker-report-import", "consultation-observe", "campaign-record", "campaign-release", "phase-gate-record", "phase-gate-advance", "worker-accept", "worker-reject", "task-finalize", "control-finalize", "control-archive",
];
...
function validateTransitionReceipt(value, name) {
  ...
  oneOf(value.subject.kind, ["control", "task", "registry-observation", "worker-run", "consultation", "campaign", "phase-gate", "task-finalization"], `${name}.subject.kind`);
...
export function validateManifest(value) {
  exact(value, ["schema_version", "record_revision", "control_id", "status", "declaration", "continuation", "durability", "budget", "role_effect_policy", "document_refs", "registry_observations", "tasks", "task_cancellations", "worker_runs", "consultations", "campaigns", "phase_gate", "task_finalizations", "control_finalization", "transition_receipts", "last_update"], "manifest");
  ...
  for (let index = 0; index < value.transition_receipts.length; index++) {
    ...
    if (["placement-reserve", "worker-run-record", "worker-workspace-bind", "phase-gate-record"].includes(receipt.operation) !== (receipt.subject_digest !== null)) fail("INVALID_SCHEMA", "transition subject digest is invalid for operation");
  }
...
export async function init(input) {
  ...
  return withLock(identity, async (paths) => {
    ...
    const manifest = {
      ...
      budget: structuredClone(input.budget), role_effect_policy: structuredClone(ROLE_EFFECT_POLICY), document_refs: [...input.document_refs], registry_observations: [], tasks: [], task_cancellations: [], worker_runs: [], consultations: [], campaigns: [], phase_gate: null, task_finalizations: [], control_finalization: null,
...
function briefManifest(manifest) {
  ...
  return {
    schema_version: "dotagents.orchestration-status-brief.v4",
...
export async function resumeCheck(input) {
  ...
  return withLock(identity, async (paths) => {
    ...
    return {
      schema_version: "dotagents.orchestration-resume-check.v3", checked_at: new Date().toISOString(),
