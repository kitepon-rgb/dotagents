...
const MANIFEST_SCHEMA = "dotagents.orchestration-control.v12";
...
function validatePlacementCandidate(value, name) {
  exact(value, ["candidate_id", "registry_observation_id", "assignment_id", "workspace_cwd", "write_mode", "operation_digest", "budget_reservation", "lineage", "executor_handle"], name);
...
function validateWorker(value, stored = true) {
  ...
  exact(value, [...common, ...location, "workflow_capabilities", "budget_reservation", "write_mode", "operation_digest", "execution_verification", "lineage", "state", "executor_handle", "executor_observation", "admission", "dispatch_evidence", "dispatch_attempt_evidence", "terminal_evidence", "result", "acceptance"], "worker_run");
...
const TRANSITION_OPERATIONS = [
  "control-init", "task-record", "registry-observation-record", "worker-run-record", "consultation-record", "worker-admit",
  "worker-observe", "consultation-observe", "worker-accept", "worker-reject", "task-finalize", "control-finalize", "control-archive",
];
...
function validateTransitionReceipt(value, name) {
  ...
  oneOf(value.subject.kind, ["control", "task", "registry-observation", "worker-run", "consultation", "task-finalization"], `${name}.subject.kind`);
...
export function validateManifest(value) {
  exact(value, ["schema_version", "record_revision", "control_id", "status", "declaration", "continuation", "durability", "budget", "role_effect_policy", "document_refs", "registry_observations", "tasks", "worker_runs", "consultations", "task_finalizations", "control_finalization", "transition_receipts", "last_update"], "manifest");
  ...
  unique(value.registry_observations, "registry_observation_id"); unique(value.tasks, "task_id"); unique(value.worker_runs, "worker_run_id"); unique(value.consultations, "consultation_id"); unique(value.task_finalizations, "task_id");
...
export async function init(input) {
  ...
  return withLock(identity, async (paths) => {
    ...
    const manifest = {
      ...
      declaration: { objective_ref: input.objective_ref, project_root_realpath: identity.projectRoot, common_dir_realpath: identity.commonDir, git_dir_realpath: identity.gitDir, base_sha: identity.kind === "bare" ? null : identity.head, initial_dirty: dirty, created_at: now, created_by: input.actor_id }, continuation, durability: structuredClone(DURABILITY_PROTOCOL),
      budget: structuredClone(input.budget), role_effect_policy: structuredClone(ROLE_EFFECT_POLICY), document_refs: [...input.document_refs], registry_observations: [], tasks: [], worker_runs: [], consultations: [], task_finalizations: [], control_finalization: null,
...
const placementReason = new Map([
  ...
  ["INVALID_SCHEMA", "candidate-invalid"],
]);
...
function registryRefreshState(manifests, registry, evaluatedAt) {
  ...
  const latestTime = Math.max(...snapshots.map((entry) => Date.parse(entry.verification.observed_at)));
...
export async function placementDryRun(input) {
  ...
  return withLock(identity, async (paths) => {
    ...
    for (const candidate of [...input.candidates].sort((left, right) => left.candidate_id.localeCompare(right.candidate_id))) {
      ...
      if (!registry) { hard.push("registry-missing"); results.push(placementResult(candidate, hard, review)); continue; }
      const refresh = registryRefreshState(manifests, registry, input.evaluated_at);
      ...
      if (Date.parse(registry.expires_at) <= Date.parse(input.evaluated_at)) hard.push("registry-expired");
      ...
      try {
        ...
        const synthetic = {
          ...
          workspace_cwd: candidate.workspace_cwd, workflow_capabilities: structuredClone(registry.workflow_capabilities),
          ...
          lineage: structuredClone(candidate.lineage), state: "planned", executor_handle: structuredClone(candidate.executor_handle),
        ...
        if (task.isolation === "dedicated-worktree" && (workspace.kind !== "worktree" || workspace.worktreeRoot === manifest.declaration.project_root_realpath)) fail("WORKSPACE_DRIFT", "dedicated worktree is required");
        ...
        if (task.classification === "H") {
          ...
          if (task.approval.expires_at !== null && Date.parse(task.approval.expires_at) <= Date.parse(input.evaluated_at)) fail("APPROVAL_EXPIRED", "H approval is expired");
        }
        ...
        requireDependenciesReady(manifest, task); assignmentAllows(allWorkers, synthetic.assignment_id, "worker");
        await inspectTaskScopes(workspace, task);
        const stored = structuredClone(synthetic); delete stored.workspace_cwd; stored.workspace = workspaceObject(workspace); stored.baseline_workspace_fingerprint = null;
      ...
      results.push(placementResult(candidate, hard, review));
    }
...
function validateGlobalManifests(manifests) {
  ...
  for (let leftIndex = 0; leftIndex < activeWriters.length; leftIndex++) {
    ...
    for (let rightIndex = leftIndex + 1; rightIndex < activeWriters.length; rightIndex++) {
      ...
      const sameWorktree = left.run.workspace.worktree_root_realpath === right.run.workspace.worktree_root_realpath;
...
function assertControlReadyForFinalization(manifest, code) {
  ...
  if (!workerReady || !consultationReady || !tasksFinalized) fail(code, "control is not ready for finalization");
}
