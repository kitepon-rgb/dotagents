...
const MANIFEST_SCHEMA = "dotagents.orchestration-control.v8";
...
export function validateManifest(value) {
  exact(value, ["schema_version", "record_revision", "control_id", "status", "declaration", "continuation", "budget", "role_effect_policy", "document_refs", "tasks", "worker_runs", "consultations", "task_finalizations", "control_finalization", "transition_receipts", "last_update"], "manifest");
  ...
  validateDeclaration(value.declaration); validateContinuation(value.continuation, value.control_id); validateBudget(value.budget); validateRoleEffectPolicy(value.role_effect_policy); refs(value.document_refs, "manifest.document_refs");
...
export async function init(input) {
  ...
  return withLock(identity, async (paths) => {
    ...
    const manifest = {
      ...
      declaration: { objective_ref: input.objective_ref, project_root_realpath: identity.projectRoot, common_dir_realpath: identity.commonDir, git_dir_realpath: identity.gitDir, base_sha: identity.kind === "bare" ? null : identity.head, initial_dirty: dirty, created_at: now, created_by: input.actor_id }, continuation,
      budget: structuredClone(input.budget), role_effect_policy: structuredClone(ROLE_EFFECT_POLICY), document_refs: [...input.document_refs], tasks: [], worker_runs: [], consultations: [], task_finalizations: [], control_finalization: null,
