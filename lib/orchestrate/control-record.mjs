...
const MANIFEST_SCHEMAS = Object.freeze([MANIFEST_SCHEMA_V25, MANIFEST_SCHEMA_V26, MANIFEST_SCHEMA_V27, MANIFEST_SCHEMA_V28]);
...
const explicitConsultationCancelSchema = (schemaVersion) => [MANIFEST_SCHEMA_V27, MANIFEST_SCHEMA_V28].includes(schemaVersion);
...
const MIGRATION_EDGES = Object.freeze({
  ...
  [MANIFEST_SCHEMA_V28]: [MANIFEST_SCHEMA_V27],
});
...
export function validateManifest(value) {
  ...
  if (value.schema_version !== MANIFEST_SCHEMA_V28 && value.transition_receipts.some((receipt) => receipt.operation === "artifact-generation-record")) fail("INVALID_SCHEMA", "artifact generation receipt requires a v28 manifest");
  ...
  for (const artifact of value.artifacts) {
    ...
    if (records.length === 1 && manifestSchemaAtRevision(value, records[0].revision) === MANIFEST_SCHEMA_V28) versionedArtifactRef(artifact.artifact_ref, artifact.artifact_digest, "artifact.artifact_ref");
...
export async function init(input) {
  ...
  return withLock(identity, async (paths) => {
    ...
    const manifest = {
      schema_version: MANIFEST_SCHEMA_V28, record_revision: 0, control_id: input.control_id, status: "active",
      declaration: {
...
export async function artifactRecord(input) {
  ...
  return mutation(input, (before, next) => {
    ...
    if (manifest.schema_version === MANIFEST_SCHEMA_V28) versionedArtifactRef(input.artifact.artifact_ref, input.artifact.artifact_digest, "input.artifact.artifact_ref");
...
export async function artifactGenerationRecord(input) {
  ...
  return mutation(input, (_before, next) => {
    ...
    if (manifest.schema_version !== MANIFEST_SCHEMA_V28) fail("SCHEMA_UPGRADE_REQUIRED", "artifact generation requires a v28 manifest; run control-migrate first");
