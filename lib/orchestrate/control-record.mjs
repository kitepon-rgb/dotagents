...
const MANIFEST_SCHEMAS = Object.freeze([MANIFEST_SCHEMA_V25, MANIFEST_SCHEMA_V26, MANIFEST_SCHEMA_V27]);
// Version set predicate, never equality against a single version (ADR 0054 refuter指摘: v26等値判定はv27を黙ってv25扱いする).
const typedConsultationSchema = (schemaVersion) => schemaVersion !== MANIFEST_SCHEMA_V25;
// control-migrate moves between adjacent versions only; no v25→v27 shortcut exists.
const MIGRATION_EDGES = Object.freeze({
  ...
  [MANIFEST_SCHEMA_V27]: [MANIFEST_SCHEMA_V26],
});
...
function validateConsultation(value, schemaVersion) {
  ...
  const states = schemaVersion === MANIFEST_SCHEMA_V27
...
const TRANSITION_OPERATIONS = [
  ...
  "worker-workspace-bind", "worker-observe", "worker-report-import", "consultation-observe", "consultation-cancel", "campaign-record", "campaign-release", "phase-gate-record", "phase-gate-advance", "artifact-record", "artifact-status-record", "approach-family-record", "approach-family-block", "approach-family-reopen", "worker-accept", "worker-reject", "task-finalize", "control-migrate", "control-finalize", "control-archive",
];
...
export function validateManifest(value) {
  ...
  boundedArray(value.worker_runs, "manifest.worker_runs", (entry) => validateWorker(entry, true, EXECUTOR_CONTRACT_REGISTRY, value.schema_version === MANIFEST_SCHEMA_V27));
...
async function externalStatePaths(identity, create) {
  ...
  if (create) {
    // 祖先（~/.local/state等）は所有外の共有ディレクトリなので存在だけ保証し、strict 0700検査は
    // dotagents配下＝自分の私有subtreeにだけ適用する。
    ...
    await ensureDir(join(base, "dotagents"), { create: true });
...
async function acquireLock(ownersDir) {
  ...
  try {
    ...
    const others = [];
    for (const entry of entries) {
      const found = await readOwner(join(ownersDir, entry));
...
function orphanedPlannedConsultation(manifest, consultation) {
  if (manifest.schema_version === MANIFEST_SCHEMA_V27) return false;
...
export async function init(input) {
  ...
  return withLock(identity, async (paths) => {
    ...
    const manifest = {
      schema_version: MANIFEST_SCHEMA_V27, record_revision: 0, control_id: input.control_id, status: "active",
      declaration: {
...
export async function reservePlacement(input) {
  ...
  return mutation(input, (before, next) => {
    ...
    if (input.selector_decision !== undefined && manifest.schema_version !== MANIFEST_SCHEMA_V27) fail("SCHEMA_UPGRADE_REQUIRED", "placement selector_decision requires a v27 manifest; run control-migrate first");
...
export async function consultationCancel(input) {
  ...
  return mutation(input, { operation: "consultation-cancel", subjectKind: "consultation", subjectId: input.consultation_id, previousState: "planned", nextState: "cancelled", evidence: [input.decision] }, async (manifest) => {
    if (manifest.schema_version !== MANIFEST_SCHEMA_V27) fail("SCHEMA_UPGRADE_REQUIRED", "consultation cancel requires a v27 manifest; run control-migrate first");
