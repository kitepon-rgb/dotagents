...
const MANIFEST_SCHEMAS = Object.freeze([MANIFEST_SCHEMA_V25, MANIFEST_SCHEMA_V26]);
...
function validateWorker(value, stored = true, registry = EXECUTOR_CONTRACT_REGISTRY) {
...
function validateConsultation(value, schemaVersion) {
  const handleField = schemaVersion === MANIFEST_SCHEMA_V26 ? "consultation_handle" : "slug";
  ...
  if (schemaVersion === MANIFEST_SCHEMA_V26) {
  ...
  // Bind effort to the product contract for the v26 connectors so a planned consultation can
  // always be dispatched by its request builder. gpt-connector stays opaque for v25 migration compatibility.
  if (schemaVersion === MANIFEST_SCHEMA_V26 && value.connector === "claude-native") oneOf(value.effort, ["low", "medium", "high", "xhigh", "max"], "consultation.effort");
  if (schemaVersion === MANIFEST_SCHEMA_V26 && value.connector === "codex-sidecar") oneOf(value.effort, ["low", "medium", "high", "xhigh"], "consultation.effort");
...
const TRANSITION_OPERATIONS = [
  ...
  "worker-workspace-bind", "worker-observe", "worker-report-import", "consultation-observe", "campaign-record", "campaign-release", "phase-gate-record", "phase-gate-advance", "artifact-record", "artifact-status-record", "approach-family-record", "approach-family-block", "approach-family-reopen", "worker-accept", "worker-reject", "task-finalize", "control-migrate", "control-finalize", "control-archive",
];
...
export function validateManifest(value) {
  exact(value, ["schema_version", "record_revision", "control_id", "status", "declaration", "continuation", "durability", "budget", "role_effect_policy", "document_refs", "registry_observations", "tasks", "task_cancellations", "worker_runs", "consultations", "campaigns", "phase_gate", "artifacts", "family_governance", "task_finalizations", "control_finalization", "transition_receipts", "last_update"], "manifest");
  if (!MANIFEST_SCHEMAS.includes(value.schema_version)) fail("INVALID_SCHEMA", "unsupported manifest schema");
  ...
  for (let index = 0; index < value.transition_receipts.length; index++) {
    ...
    const subjectDigestRequired = ["placement-reserve", "worker-run-record", "worker-workspace-bind", "campaign-record", "phase-gate-record", "artifact-record", "artifact-status-record", "approach-family-record", "approach-family-block", "approach-family-reopen", "task-finalize", "control-finalize"].includes(receipt.operation);
  ...
  for (const artifact of value.artifacts) {
    const receipts = value.transition_receipts.filter((receipt) => receipt.subject.kind === "artifact" && receipt.subject.id === artifact.artifact_id);
    const records = receipts.filter((receipt) => receipt.operation === "artifact-record");
    const updates = receipts.filter((receipt) => receipt.operation === "artifact-status-record").sort((left, right) => left.revision - right.revision);
    ...
    if (receipts.length !== records.length + updates.length) fail("INVALID_SCHEMA", "artifact receipt operation is invalid");
    let state = records[0].next_state;
    ...
    if (artifact.status !== state || (artifact.status === "current" ? updates.length !== 0 : updates.length !== 1)) fail("INVALID_SCHEMA", "artifact status differs from receipts");
    if (artifact.status !== "current") {
      const receipt = updates[0];
      if (receipt.revision !== artifact.status_from_revision + 1 || receipt.actor_id !== artifact.status_by || Date.parse(receipt.recorded_at) < Date.parse(artifact.status_at)) fail("INVALID_SCHEMA", "artifact status metadata is invalid");
    }
  }
  if (value.transition_receipts.filter((receipt) => receipt.operation === "artifact-record").length !== value.artifacts.length) fail("INVALID_SCHEMA", "artifact record receipts differ from artifacts");
  if (value.transition_receipts.filter((receipt) => receipt.operation === "artifact-status-record").length !== value.artifacts.filter((artifact) => artifact.status !== "current").length) fail("INVALID_SCHEMA", "artifact status receipts differ from artifacts");
...
// no planned->terminal transition, so it is exempted from closure demands instead of being rewritten
// (ADR 0053). The record keeps state "planned" for audit; dispatched and later states must still be
// collected to terminal. v27 introduces an explicit cancelled state and retires this exemption.
...
export async function init(input) {
  ...
  return withLock(identity, async (paths) => {
    ...
    const manifest = {
      schema_version: MANIFEST_SCHEMA_V26, record_revision: 0, control_id: input.control_id, status: "active",
      declaration: {
      ...
      budget: structuredClone(input.budget), role_effect_policy: structuredClone(ROLE_EFFECT_POLICY), document_refs: [...input.document_refs], registry_observations: [], tasks: [], task_cancellations: [], worker_runs: [], consultations: [], campaigns: [], phase_gate: null, artifacts: [], family_governance: [], task_finalizations: [], control_finalization: null,
...
function briefManifest(manifest) {
  ...
  const activeConsultations = manifest.consultations.filter((entry) => CONSULT_NONTERMINAL.has(entry.state)).map((entry) => ({
    ...
    consultation_handle: manifest.schema_version === MANIFEST_SCHEMA_V26 ? structuredClone(entry.consultation_handle) : { slug: entry.slug },
...
async function evidenceRetention(manifest, identity) {
  ...
  for (const descriptor of descriptors) {
    ...
    try {
      ...
      const retainedInHistory = descriptor.type === "decision" && observed !== descriptor.digest
...
export async function consultationRecord(input) {
  ...
  const inputSchemaVersion = Object.hasOwn(input.consultation, "consultation_handle") ? MANIFEST_SCHEMA_V26 : MANIFEST_SCHEMA_V25;
  validateConsultation(input.consultation, inputSchemaVersion);
  ...
  return mutation(input, { operation: "consultation-record", subjectKind: "consultation", subjectId: input.consultation.consultation_id, previousState: null, nextState: "planned", evidence: [] }, async (manifest, manifests) => {
    if (manifest.schema_version !== inputSchemaVersion) {
      if (inputSchemaVersion === MANIFEST_SCHEMA_V26) fail("SCHEMA_UPGRADE_REQUIRED", "typed consultation_handle requires a v26 manifest; run control-migrate first");
      fail("INVALID_SCHEMA", "v26 manifest consultation requires consultation_handle");
    }
...
// Explicit one-shot schema migration (ADR 0045). Mutation-time auto-upgrade is forbidden;
// finalized controls are blocked by CONTROL_FINALIZED and archived controls by RECORD_ARCHIVED.
export async function controlMigrate(input) {
  ...
  return mutation(input, (before) => ({
    ...
    if (input.target_schema_version === MANIFEST_SCHEMA_V26) {
      for (const consultation of manifest.consultations) {
...
export async function observeConsultation(input) {
  ...
  return mutation(input, (before) => {
    ...
    if (Object.hasOwn(observation, "consultation_handle")) {
      const expectedHandle = manifest.schema_version === MANIFEST_SCHEMA_V26 ? consultation.consultation_handle : { slug: consultation.slug };
