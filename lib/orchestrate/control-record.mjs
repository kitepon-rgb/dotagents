...
const MANIFEST_SCHEMA = "dotagents.orchestration-control.v17";
...
function validateWorkspaceBinding(value) {
  ...
  exact(value, ["mode", "schema_version", "base_sha", "preserve_worktree", "execution_workspace", "bound_from_revision", "binding_evidence", "bound_by", "bound_at"], "workspace_binding");
  ...
  if (value.execution_workspace === null) {
    if (value.bound_from_revision !== null || value.bound_by !== null || value.bound_at !== null || value.binding_evidence.length !== 0) fail("INVALID_SCHEMA", "unbound executor workspace has binding metadata");
  } else {
...
export function validateManifest(value) {
  ...
  for (const run of value.worker_runs) {
    ...
    if (run.workspace_binding.mode === "fixed" || run.workspace_binding.execution_workspace === null) {
      ...
      if (bindReceipt.revision !== run.workspace_binding.bound_from_revision + 1 || bindReceipt.actor_id !== run.workspace_binding.bound_by || bindReceipt.previous_state !== bindReceipt.next_state || !["dispatched", "running", "unknown"].includes(bindReceipt.previous_state) || Date.parse(bindReceipt.recorded_at) < Date.parse(run.workspace_binding.bound_at) || canonicalJson(bindReceipt.evidence) !== canonicalJson(run.workspace_binding.binding_evidence)) fail("INVALID_SCHEMA", "workspace bind receipt is invalid");
    }
  }
...
export async function bindWorkerWorkspace(input) {
  validateMutationBase(input, ["worker_run_id", "workspace_cwd", "binding_evidence"]);
  identifier(input.worker_run_id, "input.worker_run_id"); string(input.workspace_cwd, "input.workspace_cwd", 4096); evidenceArray(input.binding_evidence, "input.binding_evidence", 1);
  let pathInfo;
  ...
  return mutation(input, (before) => {
    ...
    return { operation: "worker-workspace-bind", subjectKind: "worker-run", subjectId: input.worker_run_id, previousState: run?.state ?? null, nextState: run?.state ?? "missing", evidence: input.binding_evidence };
  }, async (manifest) => {
    const run = manifest.worker_runs.find((entry) => entry.worker_run_id === input.worker_run_id); if (!run) fail("INVALID_SCHEMA", "worker run does not exist");
