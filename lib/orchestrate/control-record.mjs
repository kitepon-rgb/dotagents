...
const MANIFEST_SCHEMA = "dotagents.orchestration-control.v1";
...
function validateTask(value, stored = true) {
  const keys = ["task_id", "title", "classification", "effect", "doc_ref", ...(stored ? ["doc_content_oid"] : []), "read_scope", "write_scope", "approval_ref", "alternative_group"];
  ...
  if (stored && !SHA1_RE.test(value.doc_content_oid)) fail("INVALID_SCHEMA", "task.doc_content_oid is invalid");
...
function validateAcceptance(value, executor) {
  ...
  validateHandle(value.executor_handle, executor, true);
...
function validateWorker(value, stored = true) {
  const common = ["worker_run_id", "task_id", "assignment_id", "executor", "role_ref"];
  const location = stored ? ["workspace", "baseline_workspace_fingerprint"] : ["workspace_cwd"];
  exact(value, [...common, ...location, "write_mode", "execution_verification", "state", "executor_handle", "executor_observation", "admission", "dispatch_evidence", "dispatch_attempt_evidence", "terminal_evidence", "result", "acceptance"], "worker_run");
  ...
  oneOf(value.executor, ["parent", "codex-native", "codex-sidecar", "aiterm", "claude-native"], "worker_run.executor");
  ...
  validateHandle(value.executor_handle, value.executor, true);
  ...
  if (stored) {
    ...
    if (value.acceptance !== null) validateAcceptance(value.acceptance, value.executor);
...
function validateConsultation(value) {
  exact(value, ["consultation_id", "task_id", "assignment_id", "connector", "slug", "model", "effort", "state", "executor_observation", "decision_ref", "terminal_evidence"], "consultation");
...
export function validateManifest(value) {
  exact(value, ["schema_version", "record_revision", "control_id", "status", "declaration", "document_refs", "tasks", "worker_runs", "consultations", "task_finalizations", "last_update"], "manifest");
  ...
  validateDeclaration(value.declaration); refs(value.document_refs, "manifest.document_refs");
...
async function mutation(input, mutate) {
  ...
  return withLock(identity, async (paths) => {
    ...
    next.record_revision += 1; next.last_update = { actor_id: input.actor_id, updated_at: new Date().toISOString() };
...
async function documentOid(identity, docRef) {
  ...
  if (identity.kind === "bare") {
    ...
    const oid = result.stdout.toString().trim(); if (result.code !== 0 || !SHA1_RE.test(oid)) fail("IO_FAILURE", "task document is unavailable"); return oid;
  }
  ...
  const oid = result.stdout.toString().trim(); if (!SHA1_RE.test(oid)) fail("GIT_FAILURE", "git returned invalid document oid"); return oid;
}
...
export async function init(input) {
  apiInput(input, ["cwd", "control_id", "objective_ref", "actor_id", "document_refs"]);
  ...
  return withLock(identity, async (paths) => {
    ...
    const manifest = {
      ...
      document_refs: [...input.document_refs], tasks: [], worker_runs: [], consultations: [], task_finalizations: [], last_update: { actor_id: input.actor_id, updated_at: now },
    };
...
export async function taskRecord(input) {
  ...
  return mutation(input, async (manifest, manifests, identity) => {
    ...
    const oid = await documentOid(identity, input.task.doc_ref); manifest.tasks.push({ ...structuredClone(input.task), doc_content_oid: oid });
  });
}
...
export async function workerRunRecord(input) {
  ...
  if (input.worker_run?.executor === "gpt-connector") fail("EXECUTOR_FORBIDDEN", "gpt-connector is consultation-only");
  validateWorker(input.worker_run, false);
  const workspaceIdentity = await gitIdentity(input.worker_run.workspace_cwd);
  return mutation(input, async (manifest, manifests) => {
    ...
    if (task.classification === "F" && task.effect === "write" && input.worker_run.executor !== "parent") fail("EXECUTOR_FORBIDDEN", "F write task must use parent");
    const rank = verificationRank.get(input.worker_run.execution_verification.stage);
    if (input.worker_run.executor !== "parent" && rank < 3) fail("VERIFICATION_REQUIRED", "external executor must be verified");
    if (task.effect === "write" && input.worker_run.executor !== "parent" && rank !== 4) fail("VERIFICATION_REQUIRED", "external writer must be execution-verified");
...
export async function consultationRecord(input) {
  ...
  return mutation(input, async (manifest, manifests) => {
    ...
    assignmentAllows(all, input.consultation.assignment_id, "consultation"); manifest.consultations.push(structuredClone(input.consultation));
  });
}
...
export async function admitWorker(input) {
  ...
  return mutation(input, async (manifest, manifests) => {
    ...
    const docIdentity = manifest.declaration.project_root_realpath === null ? await gitIdentity(input.cwd) : await gitIdentity(manifest.declaration.project_root_realpath);
    if (await documentOid(docIdentity, task.doc_ref) !== task.doc_content_oid) fail("TASK_DOCUMENT_DRIFT", "task document changed");
...
function validateWorkerObservation(value, executor) {
  ...
  if (own(value, "executor_handle")) validateHandle(value.executor_handle, executor, true);
...
export async function observeWorker(input) {
  ...
  return mutation(input, async (manifest) => {
    const run = manifest.worker_runs.find((entry) => entry.worker_run_id === input.worker_run_id); if (!run) fail("INVALID_SCHEMA", "worker run does not exist");
    validateWorkerObservation(input.observation, run.executor); const observation = input.observation;
...
export async function conflictCheck(input) {
  ...
  if (input.proposed_worker_run !== undefined) {
    validateWorker(input.proposed_worker_run, false); const task = manifest.tasks.find((entry) => entry.task_id === input.proposed_worker_run.task_id); if (!task) fail("INVALID_SCHEMA", "proposed worker task missing");
