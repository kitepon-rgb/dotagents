...
const TODO_STATUS_SCHEMA = "lattice.todo_status_result.v6";
// Control manifestの`external_source.contract_version`は、束縛した時点のwire版を書き残した
// 履歴であって、いま観測しているwire版ではない。ここを`TODO_STATUS_SCHEMA`と同一視すると、
// 旧版時代に束縛した既存Controlがbump直後に「束縛が1件も無い」へ落ちる。観測は現行版だけを
// 受理し、束縛照合は過去版も受理する——2つは別の軸である。**過去版は消さない。**
const TODO_STATUS_BINDING_CONTRACT_VERSIONS = new Set([
  "lattice.todo_status_result.v4", "lattice.todo_status_result.v5", TODO_STATUS_SCHEMA,
]);
...
const PLAN_NOTE_KEYS = Object.freeze([
  "plan_key", "plan_note_head_digest", "count", "latest", "next_commands",
]);
const PLAN_NOTE_LATEST_KEYS = Object.freeze(["event_digest", "actor_agent", "recorded_at"]);
const COORDINATION_KEYS = Object.freeze([
  "plan_key", "mode", "declared_by", "declared_at", "reason",
]);
const COORDINATION_MODES = Object.freeze(["witness", "conversation"]);
const PARALLEL_CANDIDATE_KEYS = Object.freeze([
  "plan_key", "coverage", "unjudged_task_ids", "verified_parallel_groups",
  "serialize_pairs", "next_commands",
]);
const INDEPENDENCE_COVERAGE = Object.freeze(["verified", "stale", "superseded", "missing"]);
const AUDIT_PENDING_KEYS = Object.freeze([
  "plan_key", "phase_id", "phase_status", "implicit", "required_evidence_slots", "next_commands",
]);
const AUDIT_PENDING_PHASE_STATUSES = Object.freeze(["gate_ready", "reviewing", "rejected"]);
...
function validateTodoStatus(value) {
  ...
  // v5で加わった監査待ちPhase欄。Phase entryなので`status`ではなく`phase_status`を持つ。
  // sagaはここを選択・束縛へ使わない（監査はdispatchではない）。受理して素通しさせるための検証。
  boundedArray(value.audit_pending, "todo_status.audit_pending", MAX_CONTROL_TASKS, (entry, name) => {
    exact(entry, AUDIT_PENDING_KEYS, name);
    identifier(entry.plan_key, `${name}.plan_key`);
    identifier(entry.phase_id, `${name}.phase_id`);
    if (!AUDIT_PENDING_PHASE_STATUSES.includes(entry.phase_status)) {
      fail("INVALID_OBSERVATION", `${name}.phase_status is invalid`);
    }
    if (typeof entry.implicit !== "boolean") fail("INVALID_OBSERVATION", `${name}.implicit must be a boolean`);
    boundedArray(entry.required_evidence_slots, `${name}.required_evidence_slots`, MAX_CONTROL_TASKS,
      (slot, slotName) => identifier(slot, slotName));
    boundedArray(entry.next_commands, `${name}.next_commands`, MAX_CONTROL_TASKS,
      (command, commandName) => boundedText(command, commandName, { limit: 16_384 }), { min: 1 });
  });
  // v6の3欄。sagaはどれも選択・束縛へ使わない——工程に属する事実であってdispatchではない
  // （ADR 0160）。受理して素通しさせるための検証であり、**素通しと無検証は別物**である。
  boundedArray(value.plan_notes, "todo_status.plan_notes", MAX_CONTROL_TASKS, (entry, name) => {
    exact(entry, PLAN_NOTE_KEYS, name);
    identifier(entry.plan_key, `${name}.plan_key`);
    digest(entry.plan_note_head_digest, `${name}.plan_note_head_digest`);
    safeInteger(entry.count, `${name}.count`, { min: 1 });
    boundedArray(entry.latest, `${name}.latest`, MAX_CONTROL_TASKS, (item, itemName) => {
      exact(item, PLAN_NOTE_LATEST_KEYS, itemName);
      digest(item.event_digest, `${itemName}.event_digest`);
      boundedText(item.actor_agent, `${itemName}.actor_agent`, { limit: 160 });
      boundedText(item.recorded_at, `${itemName}.recorded_at`, { limit: 64 });
    }, { min: 1 });
    // 先頭がheadであることはLattice側の不変。ここで確かめないと「最新がどれか」を
    // 消費側が自分で決めることになる。
    if (entry.latest[0].event_digest !== entry.plan_note_head_digest) {
      fail("INVALID_OBSERVATION", `${name}.latest[0] must be the chain head`);
    }
    boundedArray(entry.next_commands, `${name}.next_commands`, MAX_CONTROL_TASKS,
      (command, commandName) => boundedText(command, commandName, { limit: 16_384 }), { min: 1 });
  });
  boundedArray(value.coordination, "todo_status.coordination", MAX_CONTROL_TASKS, (entry, name) => {
    exact(entry, COORDINATION_KEYS, name);
    identifier(entry.plan_key, `${name}.plan_key`);
    if (!COORDINATION_MODES.includes(entry.mode)) {
      fail("INVALID_OBSERVATION", `${name}.mode is invalid`);
    }
    exact(entry.declared_by, ["host", "session", "agent"], `${name}.declared_by`);
    for (const key of ["host", "session", "agent"]) {
      boundedText(entry.declared_by[key], `${name}.declared_by.${key}`, { limit: 160 });
    }
    boundedText(entry.declared_at, `${name}.declared_at`, { limit: 64 });
    boundedText(entry.reason, `${name}.reason`, { limit: 512 });
  });
  boundedArray(value.parallel_candidates, "todo_status.parallel_candidates", MAX_CONTROL_TASKS, (entry, name) => {
    exact(entry, PARALLEL_CANDIDATE_KEYS, name);
    identifier(entry.plan_key, `${name}.plan_key`);
    if (!INDEPENDENCE_COVERAGE.includes(entry.coverage)) {
      fail("INVALID_OBSERVATION", `${name}.coverage is invalid`);
    }
    boundedArray(entry.unjudged_task_ids, `${name}.unjudged_task_ids`, MAX_CONTROL_TASKS,
      (taskId, taskName) => identifier(taskId, taskName));
    boundedArray(entry.verified_parallel_groups, `${name}.verified_parallel_groups`, MAX_CONTROL_TASKS,
      (group, groupName) => {
        exact(group, ["task_ids"], groupName);
        boundedArray(group.task_ids, `${groupName}.task_ids`, MAX_CONTROL_TASKS,
          (taskId, taskName) => identifier(taskId, taskName), { min: 1 });
      });
    boundedArray(entry.serialize_pairs, `${name}.serialize_pairs`, MAX_CONTROL_TASKS,
      (pair, pairName) => {
        exact(pair, ["task_ids", "type", "detail"], pairName);
        boundedArray(pair.task_ids, `${pairName}.task_ids`, 2,
          (taskId, taskName) => identifier(taskId, taskName), { min: 2 });
        boundedText(pair.type, `${pairName}.type`, { limit: 160 });
        boundedText(pair.detail, `${pairName}.detail`, { limit: 512 });
      });
    boundedArray(entry.next_commands, `${name}.next_commands`, MAX_CONTROL_TASKS,
      (command, commandName) => boundedText(command, commandName, { limit: 16_384 }), { min: 1 });
  });
...
function boundControlTask(status, manifest, task) {
  ...
  const matches = manifest.tasks.filter((entry) => entry.external_source !== null
    ...
    && TODO_STATUS_BINDING_CONTRACT_VERSIONS.has(entry.external_source.contract_version)
...
function inferTaskRef(status, manifest, todoId) {
  ...
  const bound = manifest.tasks.flatMap((entry) => {
    ...
    if (source === null || source.namespace !== EXTERNAL_SOURCE_NAMESPACE
      || !TODO_STATUS_BINDING_CONTRACT_VERSIONS.has(source.contract_version)) return [];
