...
const FAILURE_CODES = Object.freeze({
  ADAPTER_CREDENTIAL_MISSING: "credential-missing", ADAPTER_RATE_LIMITED: "rate-limited", ADAPTER_TIMEOUT: "timeout",
  ADAPTER_NON_ZERO_EXIT: "non-zero-exit", ADAPTER_MALFORMED_REPORT: "malformed-report", ADAPTER_WORKSPACE_MISSING: "workspace-missing", ADAPTER_UNSUPPORTED_CAPABILITY: "unsupported-capability",
});
const ADAPTER_FAILURE_SUPPORT = Object.freeze({
  "codex-sidecar": Object.freeze({ "credential-missing": true, "rate-limited": true, timeout: true, "non-zero-exit": true, "malformed-report": true, "workspace-missing": true, "unsupported-capability": true, timeout_recovery_operation: "result" }),
  "codex-native": Object.freeze({ "credential-missing": false, "rate-limited": false, timeout: true, "non-zero-exit": false, "malformed-report": false, "workspace-missing": false, "unsupported-capability": true, timeout_recovery_operation: null }),
  aiterm: Object.freeze({ "credential-missing": true, "rate-limited": true, timeout: true, "non-zero-exit": false, "malformed-report": false, "workspace-missing": true, "unsupported-capability": true, timeout_recovery_operation: "pty_read" }),
  "gpt-connector": Object.freeze({ "credential-missing": true, "rate-limited": true, timeout: true, "non-zero-exit": false, "malformed-report": true, "workspace-missing": false, "unsupported-capability": true, timeout_recovery_operation: "sessions" }),
  "claude-internal": Object.freeze({ "credential-missing": false, "rate-limited": false, timeout: false, "non-zero-exit": false, "malformed-report": false, "workspace-missing": false, "unsupported-capability": true, timeout_recovery_operation: null }),
});
...
export function lookupAdapterFailureSupport({ adapter_id, failure_family }) {
  ...
  if (!Object.values(FAILURE_CODES).includes(failure_family)) fail("FAILURE_FAMILY_UNKNOWN", "adapter failure family is unknown");
  const support = ADAPTER_FAILURE_SUPPORT[adapter_id];
  return { adapter_id, failure_family, applicable: support[failure_family], timeout_recovery_operation: failure_family === "timeout" ? support.timeout_recovery_operation : null };
}

export function projectAdapterFailure(input) {
  exact(input, ["adapter_id", "failure_code"], "adapter failure"); identifier(input.adapter_id, "adapter failure.adapter_id"); boundedString(input.failure_code, "adapter failure.failure_code", 128);
  const failure_family = FAILURE_CODES[input.failure_code];
  if (failure_family === undefined) fail("FAILURE_CODE_UNKNOWN", "adapter failure code is unknown");
  const support = lookupAdapterFailureSupport({ adapter_id: input.adapter_id, failure_family });
  if (!support.applicable) fail("FAILURE_UNSUPPORTED", "adapter failure family is not applicable");
  return {
    schema_version: ADAPTER_FAILURE_SCHEMA, adapter_id: input.adapter_id, failure_code: input.failure_code,
    state: failure_family === "timeout" ? "unknown" : "failed",
...
export function projectGptConnectorObservation(value) {
  ...
  const state = value.provider.state === "succeeded" ? "completed" : value.provider.state === "failed" ? "failed" : "running";
...
function aitermHandle(value, name = "aiterm handle") {
  exact(value, ["session_id", "agent_kind", "workspace_cwd"], name);
  aitermSessionId(value.session_id, `${name}.session_id`); aitermAgentKind(value.agent_kind, `${name}.agent_kind`); aitermWorkspace(value.workspace_cwd, `${name}.workspace_cwd`);
  return value;
}
...
export function projectAitermLaunchObservation(value) {
  ...
  const handle = aitermHandle(value, "aiterm launch observation");
  return { schema_version: AITERM_OBSERVATION_SCHEMA, executor_handle: structuredClone(handle), state: "running", terminal: null };
}
