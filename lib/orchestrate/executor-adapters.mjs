...
export function projectClaudeNativeConsultObservation(value) {
  exact(value, ["handle", "status"], "claude native consult observation");
  ...
  return { schema_version: CLAUDE_NATIVE_CONSULT_OBSERVATION_SCHEMA, consultation_handle: structuredClone(value.handle), state: value.status, raw_status: value.status, terminal: null };
}
...
function consultationProjectionCore(projection) {
  ...
  if (projection.schema_version === GPT_CONNECTOR_OBSERVATION_SCHEMA) {
    ...
    return { source: "gpt-connector", state: projection.state, raw_state: projection.raw_status };
  }
  if (projection.schema_version === CLAUDE_NATIVE_CONSULT_OBSERVATION_SCHEMA) {
    ...
    return { source: "claude-native", state: projection.state, raw_state: projection.raw_status };
  }
  if (projection.schema_version === SIDECAR_CONSULT_OBSERVATION_SCHEMA) {
    ...
    return { source: "codex-sidecar", state: projection.state, raw_state: projection.raw_status };
  }
...
export function buildConsultationControlObservation(input) {
  ...
  const output = { state, source: core.source, observed_version: input.observed_version, observed_at: input.observed_at, raw_state: core.raw_state };
  if (state === "completed") {
