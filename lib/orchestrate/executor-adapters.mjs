...
export function buildWorkerControlObservation(input) {
  ...
  if (core.state === "dispatched") {
    ...
    if (core.source === "aiterm") {
      const projection = input.projection;
      exact(projection, ["schema_version", "executor_handle", "state", "report_ref", "evidence_refs"], "completed aiterm projection");
      if (projection.report_ref === null) fail("EVIDENCE_REQUIRED", "completed aiterm projection requires Worker Report reference");
      boundedString(projection.report_ref, "completed aiterm projection.report_ref", 1024);
      boundedArray(projection.evidence_refs, "completed aiterm projection.evidence_refs", 64, (entry, name) => boundedString(entry, name, 1024));
      if (new Set(projection.evidence_refs).size !== projection.evidence_refs.length) fail("INVALID_SCHEMA", "completed aiterm projection.evidence_refs contains duplicates");
      fail("WORKER_REPORT_IMPORT_REQUIRED", "completed aiterm projection must be confirmed by strict Worker Report import");
    }
