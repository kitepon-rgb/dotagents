#!/usr/bin/env node
// v4 owns this namespace exclusively.  In particular, it never enumerates v1's outbox.
...
function diagnostic(message) { process.stderr.write(`[factory-reporter-v4] ${message}\n`); }
...
function defaultStatePath() { return platform() === 'win32' ? join(process.env.LOCALAPPDATA || join(homedir(), 'AppData', 'Local'), 'dotagents', 'factory-reporter-v4') : join(process.env.XDG_STATE_HOME || join(homedir(), '.local', 'state'), 'dotagents', 'factory-reporter-v4'); }
...
function v4Endpoint(config) { if (!config.reporting.enabled) return; let url; try { url = new URL(config.reporting.endpoint); } catch { throw new Error('v4 endpointが不正です'); } if (!['http:', 'https:'].includes(url.protocol) || url.pathname !== '/api/factory/v4/reports' || url.search || url.hash) throw new Error('v4 endpointが必要です'); }
...
function envelope(bytes, acknowledgements) { return acknowledgements ? Buffer.from(JSON.stringify({ schema_version: 'dotagents.factory-outbox.v4', report_id: acknowledgements.report_id, report_base64: bytes.toString('base64'), acknowledgements })) : bytes; }
function decode(stored) { let parsed; try { parsed = JSON.parse(UTF8.decode(stored)); } catch { throw new Error('malformed'); } if (parsed?.schema_version !== 'dotagents.factory-outbox.v4') { validateReportV4(parsed); return { bytes: stored, report: parsed, acknowledgements: null }; } exact(parsed, ['schema_version', 'report_id', 'report_base64', 'acknowledgements'], 'outbox envelope'); if (typeof parsed.report_base64 !== 'string') throw new Error('malformed'); const bytes = Buffer.from(parsed.report_base64, 'base64'); if (bytes.toString('base64') !== parsed.report_base64) throw new Error('malformed'); let report; try { report = JSON.parse(UTF8.decode(bytes)); } catch { throw new Error('malformed'); } validateReportV4(report); if (report.report_id !== parsed.report_id) throw new Error('malformed'); return { bytes, report, acknowledgements: validateAcknowledgementBundleV4(parsed.acknowledgements, parsed.report_id) }; }
...
async function postOne(config, token, loc, entry) {
  ...
  try {
    ...
    if (response.ok && body?.accepted === true && body.report_id === decoded.report.report_id) {
      if (decoded.acknowledgements) { try { await acknowledgeRuntimeErrorsV4(decoded.acknowledgements); } catch { return { action: 'keep', reason: 'ack-failed' }; } }
