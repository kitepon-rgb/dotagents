...
context='INFO: プランが承認されました。統括レーン（計画に組込済みの中断・多段の受入連鎖・複数repo書込調整・裁定証跡のいずれかが確定する戦役）なら、保存先と進捗管理はグローバル CLAUDE.md / AGENTS.md「計画文書の作法」に従います。それ以外の通常レーンは会話上の成功条件や内蔵planで足り、docs正本化は不要です。このINFO自体は作業範囲を拡張しません。'
