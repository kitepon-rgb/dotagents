#!/usr/bin/env node
// v2 owns this namespace exclusively.  In particular, it never enumerates v1's outbox.
...
import { readAndValidateReportV2, readConfig, validateReportV2 } from '../lib/factory/contract.mjs';
import { acknowledgeRuntimeErrorsV2, validateAcknowledgementBundleV2 } from '../lib/factory/runtime-errors.mjs';
...
function diagnostic(message) { process.stderr.write(`[factory-reporter-v2] ${message}\n`); }
...
function defaultStatePath() { return platform() === 'win32' ? join(process.env.LOCALAPPDATA || join(homedir(), 'AppData', 'Local'), 'dotagents', 'factory-reporter-v2') : join(process.env.XDG_STATE_HOME || join(homedir(), '.local', 'state'), 'dotagents', 'factory-reporter-v2'); }
...
function envelope(bytes, acknowledgements) { return acknowledgements ? Buffer.from(JSON.stringify({ schema_version: 'dotagents.factory-outbox.v2', report_id: acknowledgements.report_id, report_base64: bytes.toString('base64'), acknowledgements })) : bytes; }
function decode(stored) { let parsed; try { parsed = JSON.parse(UTF8.decode(stored)); } catch { throw new Error('malformed'); } if (parsed?.schema_version !== 'dotagents.factory-outbox.v2') { validateReportV2(parsed); return { bytes: stored, report: parsed, acknowledgements: null }; } exact(parsed, ['schema_version', 'report_id', 'report_base64', 'acknowledgements'], 'outbox envelope'); if (typeof parsed.report_base64 !== 'string') throw new Error('malformed'); const bytes = Buffer.from(parsed.report_base64, 'base64'); if (bytes.toString('base64') !== parsed.report_base64) throw new Error('malformed'); let report; try { report = JSON.parse(UTF8.decode(bytes)); } catch { throw new Error('malformed'); } validateReportV2(report); if (report.report_id !== parsed.report_id) throw new Error('malformed'); return { bytes, report, acknowledgements: validateAcknowledgementBundleV2(parsed.acknowledgements, parsed.report_id) }; }
...
async function postOne(config, token, loc, entry) {
  ...
  try {
    ...
    if (response.ok && body?.accepted === true && body.report_id === decoded.report.report_id) {
      if (decoded.acknowledgements) { try { await acknowledgeRuntimeErrorsV2(decoded.acknowledgements); } catch { return { action: 'keep', reason: 'ack-failed' }; } }
