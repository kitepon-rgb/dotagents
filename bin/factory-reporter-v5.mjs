...
function decode(stored) { let parsed; try { parsed = JSON.parse(UTF8.decode(stored)); } catch { throw new Error('malformed'); } if (parsed?.schema_version !== 'dotagents.factory-outbox.v5') { validateReportV5(parsed); return { bytes: stored, report: parsed, acknowledgements: null }; } exact(parsed, ['schema_version', 'report_id', 'report_base64', 'acknowledgements'], 'outbox envelope'); if (typeof parsed.report_base64 !== 'string') throw new Error('malformed'); const bytes = Buffer.from(parsed.report_base64, 'base64'); if (bytes.toString('base64') !== parsed.report_base64) throw new Error('malformed'); let report; try { report = JSON.parse(UTF8.decode(bytes)); } catch { throw new Error('malformed'); } validateReportV5(report); if (report.report_id !== parsed.report_id) throw new Error('malformed'); return { bytes, report, acknowledgements: validateAcknowledgementBundleV4(parsed.acknowledgements, parsed.report_id) }; }
